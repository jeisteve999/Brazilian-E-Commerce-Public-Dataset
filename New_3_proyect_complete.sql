...
-- Stored Procedure: Top Selling Categories by Revenue
CREATE PROCEDURE sp_TopSellingCategories @topN INT
AS
BEGIN
    SELECT TOP (@topN)
...
-- Stored Procedure: Monthly Revenue by State
CREATE PROCEDURE sp_MonthlyRevenueByState @month VARCHAR(7)
AS
BEGIN
    SELECT 
        c.customer_state,
    ...
    WHERE FORMAT(o.order_purchase_timestamp, 'yyyy-MM') = @month
    GROUP BY c.customer_state
    ORDER BY revenue DESC
END;

-- Stored Procedure: Sales Evolution Between Two Dates
CREATE PROCEDURE sp_SalesEvolution @startDate DATE, @endDate DATE
AS
BEGIN
    SELECT 
...
-- Trigger: Validate Payment Amount (No Negative Values Allowed)
...
-- Table: Order Log for Auditing New Orders
...
-- Trigger: Log New Orders into Audit Table
CREATE TRIGGER trg_LogNewOrders
ON dbo.olist_orders_dataset
AFTER INSERT
AS
BEGIN
    INSERT INTO dbo.order_log (order_id)
